...
/**
 * ACM URL generation endpoint. This endpoint is used to generate ACM URLs. Parameters for request:
 * - shortlink: The shortlink to use for the ACM URL
 * - longlink: The longlink to redirect to
 * - title: The title of the ACM URL (optional)
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  try {
    // Try to create the ACM URL
    const shorturl = await addACMURL(shortlink, longlink, title || `Klefki - ${shortlink}`);
    return res.status(200).json({message: `ACM URL created successfully! The url is ${shorturl}, redirecting to ${longlink}.`});
  } catch (e: any) {
    ...
    if (e.message == 'error:keyword') {
      try {
        ...
        return res.status(200).json({message: `ACM URL created successfully! The url is ${newURL}, redirecting to ${longlink}. The previous url was ${previousURL}.`});
      } catch (e2: any) {
        console.error(e2);
